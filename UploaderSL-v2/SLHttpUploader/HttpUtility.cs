...
namespace SLHttpUploader
{
	public class HttpUtility
	{
		...
		private event UploadedCompletedEventHandler uploadCompleted;
		public event UploadedCompletedEventHandler UploadCompleted
		{
			add { uploadCompleted += value; }
			remove { uploadCompleted -= value; }
		}

		private event ProgressReportedEventHandler progressReport;
		public event ProgressReportedEventHandler ProgressReport
		{
			add { progressReport += value; }
			remove { progressReport -= value; }
		}
		...
		public void GetRequestStream(IAsyncResult result)
		{
		...
		protected void OnUploadCompleted(bool success, Exception error)
		{
			...
			if (uploadCompleted != null)
				uploadCompleted(this, new UploadCompletedEventArgs() { Error = error, Success = success });
		}
		protected void OnProgressReport(long current, long total)
		{
			...
			Action<long, long> action = (c, t) =>
			{
				...
				if (perc == 0 || perc != lastProgressReport)
				{
					lastProgressReport = perc;
					if (progressReport != null)
						progressReport(this, arg);
				}
			};
	...
	public delegate void UploadedCompletedEventHandler(object sender, UploadCompletedEventArgs e);
	public delegate void ProgressReportedEventHandler(object sender, ProgressReportEventArgs e);
